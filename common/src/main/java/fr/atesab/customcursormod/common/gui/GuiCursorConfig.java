...
public class GuiCursorConfig extends ScreenListener {
	...
	public void render(CommonMatrixStack stack, int mouseX, int mouseY, float partialTicks) {
		...
		screen.drawRightString(stack, I18nHelper.get("cursormod.config.xhotspot") + " : ", xhotspot.getXPosition(),
				xhotspot.getYPosition() + xhotspot.getHeight() / 2 - gutils.fontHeight() / 2, Color.WHITE);
		screen.drawRightString(stack, I18nHelper.get("cursormod.config.yhotspot") + " : ", yhotspot.getXPosition(),
				yhotspot.getYPosition() + yhotspot.getHeight() / 2 - gutils.fontHeight() / 2, Color.WHITE);
		screen.drawRightString(stack, I18nHelper.get("cursormod.config.location") + " : ", cursorLocation.getXPosition(),
				cursorLocation.getYPosition() + cursorLocation.getHeight() / 2 - gutils.fontHeight() / 2, Color.WHITE);
		screen.drawRightString(stack, I18nHelper.get("cursormod.config.size") + " : ", cursorSize.getXPosition(),
				cursorSize.getYPosition() + cursorSize.getHeight() / 2 - gutils.fontHeight() / 2, Color.WHITE);
		if (syncImageSize()) {
			// 使用GuiUtils中的方法绘制预览区域
			gutils.drawPreviewArea(stack, screen, width / 2 + 36, height / 2 - 64, 128, 128);
			...
			if (cursorConfig.getxHotSpot() >= 0 && cursorConfig.getxHotSpot() < imageWidth
				...
				// 使用GuiUtils中的方法绘制热点
				gutils.drawCursorHotspot(stack, screen,
			...
			if (numImage > 1) {
				...
				gutils.drawAnimationText(stack, screen, width / 2 + 100, height / 2 + 64 + 1);
			}
			selectZone.setEnable(true);
		} else {
			screen.drawCenterString(stack, I18nHelper.get("cursormod.gui.error"), width / 2 + 100,
	...
	public void init() {
		...
		if (xhotspot instanceof CommonTextField) {
			xhotspot.getHandle().setFocused(false);
		}
		if (yhotspot instanceof CommonTextField) {
			yhotspot.getHandle().setFocused(false);
		}
		if (cursorLocation instanceof CommonTextField) {
			cursorLocation.getHandle().setFocused(false);
		}
		if (cursorSize instanceof CommonTextField) {
			cursorSize.getHandle().setFocused(false);
		}
